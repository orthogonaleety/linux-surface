This is largely a personal fork of upstream linux-surface for building Fedora 44 7.x kernels. All current work lives on branches, default master is aimed at upstream, but since such has long stalled I can't speculative merge. Anyone is welcome to use my builds but as there is no collaborative oversight to keep you safe, then as a matter of minimum transparency the builds are only available for download as the original artifacts under the workflow runs, where their link to the code that created them remains unbroken, and can be publicly inspected. To further ensure the build environment is neutral you should also note that the workers are github and not remote. For more details on obtaining a build go to a relevant [tagged release](https://github.com/orthogonaleety/linux-surface/releases) and read the release instructions and use the links under there.
